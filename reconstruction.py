...
def get_image_poses(tracking_method ,idx=None, frame_rate=None, poses=None, hand_eye=None,rvecs=None, tvecs=None, kp1_matched=None, kp2_matched=None, intrinsics=None):
    ...
    Args:
        tracking_method (str): The method used for tracking the camera poses. It can be 'EM' for extrinsic matrix,
             'aruCo' for ArUco markers, or any other value for feature-based tracking
             (pose estimation).
        idx (int or None, optional): The index of the first image pose in the poses array. This is only required for
            EM and ArUco methods, where the poses array contains multiple poses.
        frame_rate (int or None, optional):  The number of frames between the two camera poses. This is only required for EM and ArUco methods.
        poses (numpy.ndarray or None, optional): An array containing the camera poses. This is only required for EM and ArUco methods.
        hand_eye (numpy.ndarray or None, optional): A 4x4 homogeneous transformation matrix representing the hand-eye calibration. This is only required for EM method.
        rvecs (numpy.ndarray or None, optional): A 3x1 array containing the rotation vector of the camera pose. This is only required for ArUco method.
        tvecs (numpy.ndarray or None, optional): A 3x1 array containing the translation vector of the camera pose. This is only required for ArUco method.
        kp1_matched (numpy.ndarray or None, optional): An array containing the keypoints in the first image. This is only required for feature-based tracking.
        kp2_matched (numpy.ndarray or None, optional): An array containing the keypoints in the second image. This is only required for feature-based tracking.
        intrinsics ( numpy.ndarray or None, optional): A 3x3 array containing the camera intrinsic matrix. This is only required for feature-based tracking.
    ...
    if tracking_method == 'EM':
        ...
        T1_to_T2 = hand_eye @ im2_poses @ np.linalg.inv(im1_poses) @ np.linalg.inv(hand_eye)
        #T1_to_T2 = im2_poses @ np.linalg.inv(im1_poses)
        #T1_to_T2 = np.linalg.inv(hand_eye) @ im2_poses @ np.linalg.inv(im1_poses) @ hand_eye
...
def sparse_reconstruction_from_video(data_path,calibration_path,save_path,
    ...
    #for idx in [0]:
        ...
        # The following are parameters needed for bundle adjustment later on:
        ...
        # transforming 3D points back to first frame's coordinate system
        bring_back = True
        if bring_back:
...
def main():
    ...
    save_path = f'{project_path}/assets/reconstructions/{method}/{type}/{folder}'
    ...
    calibration_path = f'{project_path}/calibration/endoscope_calibration/' # mac_calibration / endoscope_calibration
