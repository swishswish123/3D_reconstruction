...
import cv2
import match_pairs
from reconstruction_utils.utils import get_matched_keypoints_sift,get_matched_keypoints_superglue, extract_rigid_body_parameters, rigid_body_parameters_to_matrix
from reconstruction_utils.reconstruction_algorithms import     triangulate_points_opencv
import matplotlib.pyplot as plt


def test_keypoints_superglue(project_path, type, folder):
    return
...
def test_EM_poses(project_path, intrinsics, distortion):
    ...
    works, R1, T1 = cv2.solvePnP(ground_truth_pnts.astype('float32'), projected_points_1_undistorted_squeezed.astype('float32'), intrinsics, distortion)
    # solve PNP on second image
    works, R2, T2 = cv2.solvePnP(ground_truth_pnts.astype('float32'), projected_points_2_undistorted_squeezed.astype('float32'), intrinsics, distortion)
...
def test_aroCo_poses(intrinsics, distortion):
    ...
    # in thus test i moved by 20cm along x between the 2 images. Z distance is 36cm
    type = 'tests'  # random / phantom / EM_tracker_calib / tests
    ...
    # solve PNP on image points 1- note that input object points and image points need to be floats for this to work
    works, R1, T1 = cv2.solvePnP(object_points.astype('float32'),
                                 kp1_matched.astype('float32'), intrinsics, distortion)
    # solve PNP on second image
    works, R2, T2 = cv2.solvePnP(object_points.astype('float32'),
    ...
    projected_points_1_undistorted = cv2.undistortPoints(kp1_matched.astype('float32'), intrinsics, distortion, None, intrinsics)
    projected_points_2_undistorted = cv2.undistortPoints(kp2_matched.astype('float32'), intrinsics, distortion, None, intrinsics)

    projected_points_1_undistorted_squeezed = projected_points_1_undistorted.squeeze(1)
    projected_points_2_undistorted_squeezed = projected_points_2_undistorted.squeeze(1)

    ############### now testing triangulation
    result_norm = triangulate_points_opencv(projected_points_1_undistorted_squeezed.T, projected_points_2_undistorted_squeezed.T, intrinsics, np.array([0,0,0]), np.array(params_norm[:3]),np.array([0,0,0]),np.array(params_norm[3:]))
    result_norm = triangulate_points_opencv(projected_points_1_undistorted_squeezed.T, projected_points_2_undistorted_squeezed.T, intrinsics, np.array([0,0,0]), np.array(params_pnp[:3]),np.array([0,0,0]),np.array(params_pnp[3:]))
    ...
    assert len(set(result_norm[:, 2].round(-2)))==1 # rounding to nearest 100, then checking how many unique values there are. If just 1 unique value then all elements equal
